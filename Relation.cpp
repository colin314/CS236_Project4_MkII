...
Relation* Relation::select1(const string& attribute, const string& value) {
    if (!scheme->attribInScheme(attribute)) {
        throw std::exception("Error in select. The desired attribute is not present in the scheme");
    }
    ...
    try {
        ...
        delete rv;
        throw std::exception("Error in Relation::select1");
    }
...
Relation* Relation::select2(const string& attribute1, const string& attribute2) {
    if (!scheme->attribInScheme(attribute1) || !scheme->attribInScheme(attribute2)) {
        throw std::exception("Error in select, desired attribute is not present in the scheme");
    }
...
Relation* Relation::rename(string oldAttribute, string newAttribute) {
    if (!scheme->attribInScheme(oldAttribute)) {
        throw std::exception("Cannot rename attribute. The given attribute is not in the scheme");
    }
    if (scheme->attribInScheme(newAttribute)) {
        throw std::exception("Cannot rename attributer. The new name is already in the scheme.");
    }
